...
- Produce an **infrastructure design** (Markdown in `docs/`, with a draw.io diagram) for hosting it on Azure.
- The design must be **Well-Architected**, **scale-to-zero where possible**, and use **private endpoints for every PaaS service except the container registry and the public frontend**.
