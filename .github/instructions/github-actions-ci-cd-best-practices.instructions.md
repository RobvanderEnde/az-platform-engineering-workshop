...
> **Workshop exception (this repo only):** This is the Azure Platform Engineering teaching workshop. **Major-version tags such as `actions/checkout@v4` and `azure/login@v2` are acceptable** in `.github/workflows/*.yml` here — they keep the chore examples short, readable, and consistent with the snippets participants copy from Microsoft Learn. Do **not** rewrite the workshop workflows to full-SHA pins, and do **not** flag tag-pinned `uses:` lines as a defect. The SHA-pinning guidance below is the production default; treat it as recommended-but-not-required for this repo.
