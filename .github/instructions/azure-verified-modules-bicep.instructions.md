...
- **Schema Information**: Use `bicep_m_get_azure_resource_type_schema` (or `bicepschema`) for resource schemas
- **Bicep Best Practices**: Use `bicep_m_get_bicep_best_practices` for authoring guidance
- **AVM Metadata**: Use `bicep_m_list_avm_metadata` to pick the latest module version and verify its parameters
- **Service Documentation**: Use `microsoft_docs_search` / `microsoft_docs_fetch` for Azure service-specific guidance
