...
> **Workshop exception:** This repo is a teaching workshop, not a production pipeline. **Major-version tags (e.g. `actions/checkout@v4`, `azure/login@v2`) are acceptable here** — they keep the chore examples readable and match what participants will copy from official Microsoft Learn docs. Do **not** rewrite chore workflows to full-SHA pins. The SHA-pinning guidance below is the production default Copilot should recommend in *other* repos.
