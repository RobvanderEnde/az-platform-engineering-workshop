...
tools:
  [ 'edit/editFiles', 'web/fetch', 'runCommands', 'terminalLastCommand', 'bicep_m_get_bicep_best_practices', 'bicep_m_list_avm_metadata', 'bicep_m_get_azure_resource_type_schema', 'todos' ]
---
...
- You follow the output from tool `#bicep_m_get_bicep_best_practices` to ensure Bicep best practices
- Double check the Azure Verified Modules input if the properties are correct using tool `#bicep_m_list_avm_metadata`
- Validate raw resource shapes with `#bicep_m_get_azure_resource_type_schema` when AVM isn't a fit
